...
const keywordSchema = new mongoose.Schema({
  channelId: { type: String, required: true }, // Added channelId field
...
// Add a new keyword (including channelId)
app.post('/api/keywords', async (req, res) => {
  const { channelId, text } = req.body;

  if (!channelId || !text) {
    return res.status(400).send('Both Channel ID and Keyword text are required.');
  }
  ...
  try {
    ...
    const newKeyword = new Keyword({ channelId, text });
